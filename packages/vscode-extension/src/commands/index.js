 ...
 */
function registerCommands(context, recorder) {
    // Add Context
    context.subscriptions.push(vscode.commands.registerCommand('ccr.addContext', async () => {
    ...
    // Add Voice Note
    context.subscriptions.push(vscode.commands.registerCommand('ccr.addVoiceNote', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No active editor');
            return;
        }
        vscode.window.showInformationMessage('Voice recording feature coming soon! For now, add text context instead.');
        // TODO: Implement voice recording
        // This would use browser's MediaRecorder API or a native module
    }));
    // View Timeline
    context.subscriptions.push(vscode.commands.registerCommand('ccr.viewTimeline', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No active editor');
            return;
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel('ccr.timeline', `Timeline: ${path.basename(filePath)}`, vscode.ViewColumn.Two, { enableScripts: true });
    ...
    // Search Context
    context.subscriptions.push(vscode.commands.registerCommand('ccr.search', async () => {
        ...
        try {
            ...
            if (selected && selected.context.fileAssociations[0]) {
                ...
                if (workspaceFolder) {
                    const uri = vscode.Uri.file(path.join(workspaceFolder.uri.fsPath, file.filePath));
    ...
    // Add Decision
    context.subscriptions.push(vscode.commands.registerCommand('ccr.addDecision', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No active editor');
            return;
    ...
    // Link Ticket
    context.subscriptions.push(vscode.commands.registerCommand('ccr.linkTicket', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No active editor');
            return;
    ...
    // Show Recent Context
    context.subscriptions.push(vscode.commands.registerCommand('ccr.showRecent', async () => {
    ...
    // Show Analytics
    context.subscriptions.push(vscode.commands.registerCommand('ccr.showAnalytics', async () => {
        try {
            const analytics = await recorder.getAnalytics();
            const panel = vscode.window.createWebviewPanel('ccr.analytics', 'Code Context Analytics', vscode.ViewColumn.One, { enableScripts: true });
    ...
    // Open Web Viewer
    context.subscriptions.push(vscode.commands.registerCommand('ccr.openWebViewer', () => {
        vscode.window.showInformationMessage('Web viewer coming soon! Use the timeline and search commands for now.');
        // TODO: Launch local web server
    }));
}
function getTimelineHtml(timeline, filePath) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
...
function getAnalyticsHtml(analytics) {
    ...
    <body>
      ...
      <table>
        ...
        ${analytics.mostContextedFiles.slice(0, 10).map((f) => `
